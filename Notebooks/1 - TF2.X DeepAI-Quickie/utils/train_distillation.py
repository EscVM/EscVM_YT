...
class DistillationClassificationTrainer():
    ...
    def compile(self, loss, metric,  optimizer, label_smoothing=0.1, log_dir='logs', bin_dir='bin'):     
        ...
        """
        self.loss = loss
        self.s_loss = tf.keras.losses.CategoricalCrossentropy(label_smoothing=0.1) # for hard distillation
        ...
        self.checkpoint_manager = tf.train.CheckpointManager(checkpoint=self.checkpoint, max_to_keep=1, directory=self.bin_dir)
    ...
    def _train_step(self, x, y_true):
        ...
        """
        if self.s_distil:
            y_pred_logits_t = tf.nn.softmax(self.teacher_model(x)/self.T, axis=-1) # teacher predictions with softmax and a certain temperature
            ...
            with tf.GradientTape() as tape:
                ...
                if self.from_logits:
                    loss = (1 - self.LAMBDA)*self.loss(y_true, y_pred_logits) + (self.T**2)*self.LAMBDA*self.k_loss(y_pred_logits_t, y_pred_distil)
                else:
                    loss = (1 - self.LAMBDA)*self.loss(y_true, y_pred_class) + (self.T**2)*self.LAMBDA*self.k_loss(y_pred_logits_t, y_pred_distil)
            ...
            with tf.GradientTape() as tape:
                ...
                if self.from_logits:
                    loss = 0.5*self.loss(y_true, y_pred_logits) + 0.5*self.s_loss(y_pred_t, y_pred_distil)
                else:
                    loss = 0.5*self.loss(y_true, y_pred_class) + 0.5*self.s_loss(y_pred_t, y_pred_distil)
